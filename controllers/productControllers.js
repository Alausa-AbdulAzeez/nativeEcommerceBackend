...
const createSingleProduct = async (req, res) => {
  const { title, supplier, imageUrl, description, product_location } = req.body
  try {
    // Check if title all parameters are provided
    if (!title || !supplier || !imageUrl || !description || !product_location) {
      return res
        .status(400)
...
module.exports = { createSingleProduct, getProducts }
